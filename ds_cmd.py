...
def execute_commands(ds_name,
    ...
    for command in commands:
        try:
            ...
            commands_printout += ">>---> " + command + " <---<<" + re.sub(r'^[^\n]*?\n', r'\n', connection.send_command(command), re.DOTALL) + "\n"
    ...
    commands_printout = re.sub(r'(.+\S)\s*$', r'\1', commands_printout, re.MULTILINE)
    commands_printout = re.sub(r'\n{2,}', '\n', commands_printout, re.MULTILINE|re.DOTALL)
...
if __name__ == "__main__":
        ...
        try:
            with open(options.command_file, "r") as command_file:
                for command_line in command_file.readlines():
                    commands_raw.append(command_line.strip('\n'))
        except IOError as e:
    ...
    for command_raw in commands_raw:
        if command_raw and not is_contains(comment_line_pattern, command_raw):
